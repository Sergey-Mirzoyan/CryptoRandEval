...
# X 3.5  Discrete Fourier Transform (Specral) Test
# X 3.6  Non-Overlapping Template Matching Test
# X 3.7 Serial Test
# X 3.8 Approximate Entropy Test
# X 3.9 Cumulative Sums Test
# X 3.10 Random Excursions Test
# X 3.11 Random Excursions Variant Test 
...
def run_nist_tests(file_path): 
    ...
    f = open(file_path,'r')
    # f = open('../1 140 231.txt','r')
    bits = ''

    for i in f:
        bits += i #read_bits_from_file(filename,bigendian)   
    bits = list(bits)
...
if __name__ == "__main__":
    sys.exit(run_nist_tests('/Users/sergeymirzoyan/Desktop/(КС)ГПСЧ/CryptoRandEval/for analyse/nums.txt'))